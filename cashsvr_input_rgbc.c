/*
 * CASH! Camera Augmented Sensing Helper
 * a multi-sensor camera helper server
 *
 * Copyright (C) 2018 AngeloGioacchino Del Regno <kholk11@gmail.com>
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#define LOG_TAG			"CASH_RGBC_INPUT"

#include <sys/poll.h>
#include <sys/epoll.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <sys/socket.h>
#include <sys/un.h>
#include <dlfcn.h>
#include <fcntl.h>
#include <stdlib.h>
#include <stdio.h>
#include <stdbool.h>
#include <string.h>
#include <pthread.h>
#include <errno.h>
#include <assert.h>
#include <string.h>
#include <unistd.h>
#include <pwd.h>
#include <linux/input.h>

#include <cutils/android_filesystem_config.h>
#include <log/log.h>

#include "cash_private.h"
#include "cash_input_common.h"
#include "cash_input_rgbc.h"
#include "cash_ext.h"

#define TCS3490_STR		"AMS TCS3490 Sensor"
#define TCS3490_ALS_ITIME	"127"
#define TCS3490_ALS_GAIN	"1"

static int tcsvl_fd;

static char *uci_rgbc_chip_power_path;
static char *uci_rgbc_power_state_path;
static bool rgbc_enabled = false;

struct cash_tcs3490 tcsvl_status;

#define UNUSED __attribute__((unused))

#define LEN_NAME	4
#define LEN_CHIP_POW	8
#define LEN_POWER_STATE	15
#define LEN_ITIME	9
#define LEN_GAIN	8

int cash_rgbc_enable(bool enable)
{
	int chip_pow_fd, power_state_fd, rc;

	if (uci_rgbc_chip_power_path == NULL || uci_rgbc_power_state_path == NULL)
		return -1;

	/* Reset the readings to start fresh */
	tcsvl_status.red = -1;
	tcsvl_status.green = -1;
	tcsvl_status.blue = -1;
	tcsvl_status.clear = -1;

	chip_pow_fd = open(uci_rgbc_chip_power_path, O_WRONLY | O_SYNC);
	if (chip_pow_fd < 0) {
		ALOGD("Cannot open %s", uci_rgbc_chip_power_path);
		return 1;
	}

	power_state_fd = open(uci_rgbc_power_state_path, O_WRONLY | O_SYNC);
	if (power_state_fd < 0) {
		ALOGD("Cannot open %s", uci_rgbc_power_state_path);
		return 1;
	}

	if (enable) {
		rc = write(chip_pow_fd, "1", 1);
	} else {
		rc = write(power_state_fd, "0", 1);
	}

	if (rc < 1) {
		ALOGW("ERROR! Cannot %sable RGBC!", enable ? "en" : "dis");
		goto end;
	}

	if (enable) {
		rc = write(power_state_fd, "1", 1);
	} else {
		rc = write(chip_pow_fd, "0", 1);
	}

	if (rc < 1) {
		ALOGW("ERROR! Cannot %sable RGBC!", enable ? "en" : "dis");
		goto end;
	}

	rc = 0;
end:
	fsync(chip_pow_fd);
	fsync(power_state_fd);
	close(chip_pow_fd);
	close(power_state_fd);
	usleep(100000);
	rgbc_enabled = enable;

	return rc;
}

static int cash_rgbc_sys_init(int devno, int plen)
{
	char* als_Itime_path;
	char* als_gain_path;
	int fd, rc;
	struct passwd *pwd;
	struct passwd *grp;
	uid_t uid;
	gid_t gid;

	uci_rgbc_chip_power_path = (char*)calloc(plen + LEN_CHIP_POW, sizeof(char));
	if (uci_rgbc_chip_power_path == NULL) {
		ALOGE("Memory exhausted. Cannot allocate.");
		return -3;
	}

	uci_rgbc_power_state_path = (char*)calloc(plen + LEN_POWER_STATE, sizeof(char));
	if (uci_rgbc_power_state_path == NULL) {
		ALOGE("Memory exhausted. Cannot allocate.");
		return -3;
	}

	pwd = getpwnam("system");
	if (pwd == NULL) {
		ALOGD("failed to get uid for system");
		return 1;
	}

	uid = pwd->pw_uid;

	grp = getpwnam("input");
	if (grp == NULL) {
		ALOGD("failed to get gid for input");
		return 1;
	}

	gid = grp->pw_gid;

	snprintf(uci_rgbc_chip_power_path, plen + LEN_CHIP_POW,
			"%s%d/chip_pow", sysfs_input_str, devno);

	if (chown(uci_rgbc_chip_power_path, uid, gid) == -1) {
		ALOGD("Cannot chown %s", uci_rgbc_chip_power_path);
		return 1;
	}

	snprintf(uci_rgbc_power_state_path, plen + LEN_POWER_STATE,
			"%s%d/als_power_state", sysfs_input_str, devno);

	if (chown(uci_rgbc_power_state_path, uid, gid) == -1) {
		ALOGD("Cannot chown %s", uci_rgbc_power_state_path);
		return 1;
	}

	als_Itime_path = (char*)calloc(plen + LEN_ITIME, sizeof(char));
	if (als_Itime_path == NULL) {
		ALOGE("Memory exhausted. Cannot allocate.");
		free(als_Itime_path);
		return -3;
	}

	snprintf(als_Itime_path, plen + LEN_ITIME,
		"%s%d/als_Itime", sysfs_input_str, devno);

	if (chown(als_Itime_path, uid, gid) == -1) {
		ALOGD("Cannot chown %s/als_Itime", als_Itime_path);
		return 1;
	}

	fd = open(als_Itime_path, O_WRONLY);
	if (fd < 0) {
		ALOGD("Cannot open %s", als_Itime_path);
		return 1;
	}

	rc = write(fd, TCS3490_ALS_ITIME, 3);
	if (rc < 3)
		ALOGW("ERROR! Cannot set Itime value for RGBC!");

	close(fd);

	als_gain_path = (char*)calloc(plen + LEN_GAIN, sizeof(char));
	if (als_gain_path == NULL) {
		ALOGE("Memory exhausted. Cannot allocate.");
		free(als_gain_path);
		return -3;
	}

	snprintf(als_gain_path, plen + LEN_GAIN,
		"%s%d/als_gain", sysfs_input_str, devno);

	if (chown(als_gain_path, uid, gid) == -1) {
		ALOGD("Cannot chown %s/als_gain", als_gain_path);
		return 1;
	}

	fd = open(als_gain_path, O_WRONLY);
	if (fd < 0) {
		ALOGD("Cannot open %s", als_gain_path);
		return 1;
	}

	rc = write(fd, TCS3490_ALS_GAIN, 1);
	if (rc < 1)
		ALOGW("ERROR! Cannot set gain value for RGBC!");

	close(fd);

	/* We're done setting permissions now, let's move back to system context */
	if (setuid(uid) == -1) {
		ALOGD("Failed to change uid");
		return 1;
	}

	return 0;
}

/* TODO: Use IOCTL EVIOCGNAME as a waaaay better way */
static int cash_input_rgbc_find_inputdev(int maxdevs, int idev_len, char* idev_name)
{
	int fd, plen, rlen, rc, i;
	int plen_xtra = 3;
	char buf[254];
	char* path;

	/*
	 * Avoid checking more than 99 input devices,
	 * because... really? :)
	 */
	if (maxdevs > 99)
		return -1;

	if (maxdevs > 9)
		plen_xtra++;

	plen = strlen(sysfs_input_str) + plen_xtra;
	path = (char*) calloc(plen + LEN_NAME, sizeof(char));
	if (path == NULL) {
		ALOGE("Out of memory. Cannot allocate.");
		rc = -1;
		goto end;
	}

	for (i = 0; i <= maxdevs; i++) {
		rc = snprintf(path, plen + LEN_NAME, "%s%d/name",
							sysfs_input_str, i);

		if (fd > 0)
			close(fd);

		fd = open(path, (O_RDONLY | O_NONBLOCK));
		if (fd < 0) {
			ALOGD("Cannot open %s", path);
			continue;
		}

		rlen = read(fd, buf, idev_len);
		if (!rlen)
			continue;

		/*
		 * If the string length doesn't match, avoid doing
		 * expensive stuff, as the string cannot be the same.
		 * Also consider some mistakes and compare it with
		 * the name length - 1.
		 * That will cover corner cases and will not evaluate
		 * erroneously positive too many times...
		 */
		if (rlen < idev_len - 1)
			continue;

		rc = strncmp(buf, idev_name, idev_len - 1);
		if (rc == 0) {
			close(fd);

			rc = cash_rgbc_sys_init(i, plen);
			if (rc < 0)
				goto end;

			return i;
		}
	}
end:
	return rc;
}

/* TODO: Move me to background thread!! */
int cash_input_rgbc_read(struct cash_tcs3490 *tcsvl_cur,
			uint16_t want_code)
{
	struct input_event evt[64];
	int retry = 0, i, len, rc;
	bool rr, rg, rb, rcl;
	uint16_t type, code;
	int32_t value;

	if (tcsvl_fd < 0)
		return -1;

again:
	rr = false;
	rg = false;
	rb = false;
	rcl = false;

	do {
		fsync(tcsvl_fd);

		rc = read(tcsvl_fd, &evt, sizeof(evt));
		if (!rc) {
			return rc;
		}

		len = rc / sizeof(struct input_event);

		for (i = 0; i < len; i++) {
			type = evt[i].type;
			code = evt[i].code;
			value = evt[i].value;

			if (type != EV_ABS)
				continue;

			switch (code)
			{
				case ABS_DISTANCE:
					if (value >= 0) {
						tcsvl_cur->red = value;
						rr = true;
					}
					break;
				case ABS_HAT1X:
					if (value >= 0) {
						tcsvl_cur->green = value;
						rg = true;
					}
					break;
				case ABS_HAT1Y:
					if (value >= 0) {
						tcsvl_cur->blue = value;
						rb = true;
					}
					break;
				case 50000:
					if (value >= 0) {
						tcsvl_cur->clear = value;
						rcl = true;
					}
					break;
				default:
					break;
			}

			if ((rr && rg && rb && rcl) || (want_code == code))
				goto done;
		}
	} while (!rr || !rg || !rb || !rcl);

done:
	if (tcsvl_cur->clear == 0) {
		if (retry < 4) {
			retry++;
			goto again;
		} else {
			return -1;
		}
	}

	return 0;
}

int cash_input_rgbc_thr_read(struct cash_tcs3490 *tcsvl_cur,
			int rgbc_fd)
{
	struct input_event evt[16];
	int i, len, rc;
	bool rr, rg, rb, rcl;
	uint16_t type, code;
	int32_t value;

	rr = false;
	rg = false;
	rb = false;
	rcl = false;

	rc = read(rgbc_fd, &evt, sizeof(evt));
	if (!rc) {
		return rc;
	}

	len = rc / sizeof(struct input_event);

	for (i = 0; i < len; i++) {
		type = evt[i].type;
		code = evt[i].code;
		value = evt[i].value;

		if (type != EV_ABS)
			continue;

		switch (code) {
			case ABS_DISTANCE:
				if (value >= 0) {
					tcsvl_cur->red = value;
					rr = true;
				}
				break;
			case ABS_HAT1X:
				if (value >= 0) {
					tcsvl_cur->green = value;
					rg = true;
				}
				break;
			case ABS_HAT1Y:
				if (value >= 0) {
					tcsvl_cur->blue = value;
					rb = true;
				}
				break;
			case 50000:
				if (value >= 0) {
					tcsvl_cur->clear = value;
					rcl = true;
				}
				break;
			default:
				break;
		}
	}

	return 0;
}

static inline bool cash_rgbc_is_val_ok(int d1, int d2, int hysteresis)
{
	int max = d2 + hysteresis;
	int min = d2 - hysteresis;

	if (d1 < max && d1 > min)
		return true;

	return false;
}

int cash_rgbc_read_inst(struct cash_tcs3490 *tcsvl_final)
{
	/* Thread not running, we'd read nothing good here! */
	if (!ucithread_run[THREAD_RGBC])
		return -1;

	/* Sensor is disabled, what are we trying to read?! */
	if (!rgbc_enabled)
		return -1;

	/* No reading available */
	if (tcsvl_status.clear < 0) {
		ALOGE("RGBC: No reading! clear %d", tcsvl_status.clear);
		return -1;
	}

	tcsvl_final->clear = tcsvl_status.clear;

	/* Return a fake score of 1 */
	return 1;
}

/*
 * cash_rgbc_thr_read_stabilized - Reads the RGBC parameters and tries to
 *			       give back a value only if it is stable.
 *
 * \param tcsvl_final - Final structure with RGBC values
 * \param runs - Maximum number of times to read the RGBC
 * \param nmatch - Number of times to match readings
 * \param sleep_ms - Delay between each read
 * \param hyst - Hysteresis, relative to the clear measurements
 *
 * \return Returns reliability of the measurement or -INT_MAX for error;
 */
int cash_rgbc_thr_read_stabilized(
	struct cash_tcs3490 *tcsvl_final,
	int runs, int nmatch, int sleep_ms, int hyst)
{
	int retry = 0, clear, score, i;

	/* Thread not running, we'd read nothing good here! */
	if (!ucithread_run[THREAD_RGBC])
		return -1;

	/* Sensor is disabled, what are we trying to read?! */
	if (!rgbc_enabled)
		return -1;

	/* Did we get called by someone who didn't read the docs? */
	if (runs < nmatch)
		runs = nmatch + 1;

again:
	score = 0;
	clear = tcsvl_status.clear;

	for (i = 0; i < runs; i++) {
		usleep(sleep_ms*1000);

		if (cash_rgbc_is_val_ok(clear, tcsvl_status.clear, hyst))
			score++;
		else
			score--;
	}

	/* Readings are very unstable! */
	if (score < 0 && retry < 4) {
		retry++;
		goto again;
	}

	tcsvl_final->clear = clear;

	return score;
}

/*
 * cash_rgbc_read_stabilized - Reads the RGBC parameters and tries to
 *			       give back a value only if it is stable.
 *
 * \param tcsvl_final - Final structure with RGBC values
 * \param runs - Maximum number of times to read the RGBC
 * \param nmatch - Number of times to match readings
 * \param sleep_ms - Delay between each read
 * \param hyst - Hysteresis, relative to the clear measurements
 *
 * \return Returns reliability of the measurement or -INT_MAX for error;
 */
int cash_rgbc_read_stabilized(
	struct cash_tcs3490 *tcsvl_final,
	int runs, int nmatch, int sleep_ms, int hyst)
{
	struct cash_tcs3490 tcsvl_cur;
	int rc, retry = 0, clear, score = 0, i;

	/* Did we get called by someone who didn't read the docs? */
	if (runs < nmatch)
		runs = nmatch + 1;

again:
	rc = cash_input_rgbc_read(&tcsvl_cur, ABS_HAT1X);
	if (rc < 0) {
		ALOGE("RGBC read failed");
		//return -INT_MAX;
	}

	clear = tcsvl_cur.clear;

	/* First run done before looping! */
	for (i = 0; i < runs; i++) {
		usleep(sleep_ms*1000);
		rc = cash_input_rgbc_read(&tcsvl_cur, ABS_HAT1X);
		if (rc < 0)
			continue;
		if (cash_rgbc_is_val_ok(clear, tcsvl_cur.clear, hyst))
			score++;
		else
			score--;
	}

	/* Readings are very unstable! */
	if (score < 0 && retry < 4) {
		retry++;
		goto again;
	}

	tcsvl_final->clear = clear;

	return score;
}

static void *cash_input_rgbc_thread(void *unusedvar UNUSED)
{
	int ret;
	int i;
	struct epoll_event pevt[10];

	cash_rgbc_enable(true);

	ALOGD("RGBC Thread started");

	while (ucithread_run[THREAD_RGBC]) {
		ret = epoll_wait(uci_pollfd[FD_RGBC], pevt,
					10, uci_pfdelay_ms[FD_RGBC]);
		for (i = 0; i < ret; i++) {
			if (pevt[i].events & EPOLLERR ||
			    pevt[i].events & EPOLLHUP ||
			    !(pevt[i].events & EPOLLIN))
				continue;

			if (uci_pollevt[FD_RGBC].data.fd)
				cash_input_rgbc_thr_read(&tcsvl_status,
					uci_pollevt[FD_RGBC].data.fd);
		}
	}

	cash_rgbc_enable(false);

	pthread_exit((void*)((int)0));
}

/* Start/stop threads */
int cash_input_rgbc_threadman(bool start, int threadno)
{
	int ret = -1;

	if (start == false) {
		ucithread_run[threadno] = false;
		return 0;
	};

	ucithread_run[threadno] = true;

	if (threadno == THREAD_RGBC) {
		ret = pthread_create(&uci_pthreads[threadno], NULL,
				cash_input_rgbc_thread, NULL);
		if (ret != 0) {
			ALOGE("Cannot create RGBC thread.");
			return -ENXIO;
		}
	}

	return ret;
}

int cash_input_rgbc_start(bool start)
{
	return cash_input_rgbc_threadman(start, THREAD_RGBC);
}

bool cash_input_is_rgbc_alive(void)
{
	return ucithread_run[THREAD_RGBC];
}

int cash_input_rgbc_init(void)
{
	int dlen, evtno, rc;
	char *devname, *devpath;

	dlen = strlen(TCS3490_STR);
	devname = (char*) calloc(dlen, sizeof(char));
	snprintf(devname, dlen, "%s", TCS3490_STR);

	evtno = cash_input_rgbc_find_inputdev(ITERATE_MAX_DEVS, dlen, devname);
	if (evtno < 0)
		return -1;

	dlen = strlen(devfs_input_str) + 3;
	if (evtno > 9)
		dlen++;

	devpath = (char*) calloc(dlen, sizeof(char));
	if (devpath == NULL)
		return -1;

	snprintf(devpath, dlen, "%s%d", devfs_input_str, evtno);


	tcsvl_fd = open(devpath, (O_RDONLY | O_NONBLOCK));
	if (tcsvl_fd < 0) {
		ALOGE("Error: cannot open the %s input device at %s.",
			devname, devpath);
		return -1;
	}

	uci_pollfd[FD_RGBC] = epoll_create1(0);
	if (uci_pollfd[FD_RGBC] == -1) {
		ALOGE("Error: Cannot create epoll descriptor");
		return -1;
	}

	uci_pfds[FD_RGBC].fd = tcsvl_fd;
	uci_pfds[FD_RGBC].events = POLLIN;
	uci_pfdelay_ms[FD_RGBC] = 1000;

	uci_pollevt[FD_RGBC].events = POLLIN;
	uci_pollevt[FD_RGBC].data.fd = tcsvl_fd;

	rc = epoll_ctl(uci_pollfd[FD_RGBC], EPOLL_CTL_ADD,
					tcsvl_fd, &uci_pollevt[FD_RGBC]);
	if (rc) {
		ALOGE("Cannot add epoll control");
		return -1;
	}

	ucithread_run[THREAD_RGBC] = false;

	return 0;
}

